...
| 服务器管理 | 增删改查 Agent 连接；纳管前测试连通；列表批量刷新状态 |
| 服务迁移 | 设置 → 服务迁移：YAML 导出/导入纳管信息（admin） |
...
截图资源目录：[docs/images/](docs/images/)（将 PNG 放入对应文件名即可在 GitHub 显示）。

### 服务列表

![服务列表](docs/images/servers-list.png)

### 服务管理

![服务管理](docs/images/server-detail.png)

### 配置编辑

![配置编辑](docs/images/config-editor.png)

## 目录结构

```
nginx-admin/
├── docs/images/                  # README 界面截图（见 docs/images/README.md）
...
POST /api/servers/export               # 批量导出 YAML（admin，body: { ids? }，空则全部）
POST /api/servers/import               # 批量导入 YAML/JSON（admin，on_conflict: skip|update）
GET  /api/servers/:id/export           # 导出单个服务 YAML（admin，迁移用）
GET  /api/servers/:id
