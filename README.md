...
| `#/settings` | 修改密码、中心备份策略（admin）、界面外观 | 已登录 |
...
backup:
  retain_per_file: 5            # 中心备份保留份数初始默认值；可在 Web「设置」中修改（admin）
```
...
GET  /api/settings                    # 中心备份保留份数（admin）
PUT  /api/settings                    # 更新中心设置（admin）
...
| `admin` | 在 editor 基础上：服务器的增删改、用户管理、中心设置 |
